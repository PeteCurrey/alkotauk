...
export default function LobbyHeader() {
  ...
  return (
    <header
      className={`fixed top-0 z-50 w-full px-4 sm:px-8 lg:px-12 xl:px-16 transition-all duration-300 font-normal ${
        isScrolled
    ...
    >
      <div className="mx-auto flex max-w-[1600px] w-full items-center justify-between gap-6 relative">
        {/* ── LEFT GROUP: Parent site context + Lobby Identity + Date (Anchored to the Left) ─────── */}
        <div className="flex items-center gap-2.5 sm:gap-3.5 shrink-0">
          <Link
            href="/"
            className="flex items-center gap-1.5 text-xs text-white/70 hover:text-white transition-colors no-underline uppercase tracking-[0.16em] font-normal whitespace-nowrap"
          >
            <ArrowLeft className="h-3.5 w-3.5 text-alkota-orange" />
          ...
          <Link
            href="/lobby"
            className="text-xs font-medium uppercase tracking-[0.22em] text-white hover:text-alkota-orange transition-colors no-underline whitespace-nowrap"
          >
            The Lobby
          </Link>

          <span className="hidden xl:inline text-white/20 font-light">|</span>

          <span className="hidden xl:inline text-[10px] font-mono text-white/40 tracking-wider whitespace-nowrap">
            {currentDate}
          </span>
        </div>
        ...
        <div className="flex items-center gap-3 sm:gap-4 font-normal justify-end shrink-0 flex-nowrap">
          <Link
