  ...
  try {
    ...
    if (!customer_name || !email) {
      return NextResponse.json(
        { error: 'Name and email are required to submit an enquiry.' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({
      ...
      message: 'Parts enquiry successfully logged. Our workshop team will review your request.',
    });
  } catch (err: any) {
    console.error('Parts enquiry endpoint error:', err);
