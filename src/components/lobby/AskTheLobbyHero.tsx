    ...
    <section className="relative min-h-[100dvh] flex flex-col overflow-hidden">
      ...
      <div className="absolute inset-0">
        <Image
          ...
          sizes="100vw"
        />
        {/* Multi-layer overlay: dark at top for header legibility, heavier at bottom for input legibility */}
        <div className="absolute inset-0 bg-gradient-to-b from-black/75 via-black/45 to-black/85" />
        {/* Subtle warm grain / vignette */}
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_100%_100%_at_50%_0%,transparent_40%,rgba(0,0,0,0.45)_100%)]" />
      </div>

      {/* ── MASTHEAD — top-centre branding ────────────────────────────────── */}
      <div className="relative z-10 flex-1 flex flex-col">
        <div className="flex-1 flex flex-col items-center justify-start pt-32 sm:pt-36 px-6 sm:px-12 text-center">
          {/* Editorial badge */}
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/5 border border-white/15 text-[#FF6900] text-[11px] font-mono uppercase tracking-[0.25em] mb-5 backdrop-blur-sm">
            <span className="h-1.5 w-1.5 rounded-full bg-[#FF6900] animate-pulse" />
            <span>Industrial Intelligence &amp; Regulatory Platform</span>
          </div>

          <h1 className="font-extralight text-5xl sm:text-7xl lg:text-8xl uppercase tracking-tight text-white leading-[0.92] mb-5 drop-shadow-[0_2px_16px_rgba(0,0,0,0.6)]">
            The Lobby.
          </h1>

          <p className="font-normal text-base sm:text-lg text-white/70 leading-relaxed max-w-2xl drop-shadow-[0_1px_8px_rgba(0,0,0,0.8)]">
        ...
        {/* ── ASK THE LOBBY CARD — anchored to bottom-centre of hero ──────── */}
        <div className="relative z-10 w-full px-4 sm:px-8 pb-12 sm:pb-16 pt-8">
          <div className="mx-auto max-w-3xl">
            {/* Card */}
            <div className="bg-black/70 backdrop-blur-xl border border-white/15 p-6 sm:p-8 shadow-2xl">
              {/* Card header */}
              <div className="flex items-center justify-between gap-4 mb-5 pb-4 border-b border-white/10">
                <div className="flex items-center gap-3">
                  <div className="flex h-8 w-8 items-center justify-center bg-[#FF6900] text-white shadow-md shrink-0">
                ...
                {answer && (
                  <button
                    onClick={handleReset}
                    className="inline-flex items-center gap-1.5 text-xs text-white/50 hover:text-white transition-colors uppercase tracking-wider font-mono bg-transparent border-none cursor-pointer shrink-0"
                  >
              ...
              <form onSubmit={e => handleSubmit(e)} className="relative">
                <div className="relative flex items-center">
                  <input
                    ...
                    disabled={loading}
                    className="w-full bg-black/50 border border-white/20 text-white placeholder-white/35 text-sm sm:text-base px-4 sm:px-5 py-4 pr-28 sm:pr-36 focus:outline-none focus:border-[#FF6900] transition-colors font-normal disabled:opacity-50"
                  />
                  <button
                    type="submit"
              ...
              {!answer && !loading && (
                <div className="mt-4 pt-4 border-t border-white/5">
                  <p className="text-[10px] uppercase font-mono tracking-widest text-white/40 mb-2.5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {PROMPT_CHIPS.map((chip, idx) => (
                      <button
                        ...
                        className="text-xs bg-white/5 hover:bg-white/10 text-white/60 hover:text-white px-3 py-1.5 border border-white/10 transition-colors cursor-pointer text-left font-normal"
                      >
                        {chip.label}
                      </button>
                    ))}
              ...
              {loading && (
                <div className="mt-6 py-8 text-center border-t border-white/10">
                  <div className="inline-flex items-center gap-3 text-sm text-white/60 font-mono">
                ...
                {answer && (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between gap-4 mb-4 pb-2 border-b border-white/5">
                      ...
                      <button
                        onClick={handleCopy}
                        className="inline-flex items-center gap-1 text-[11px] text-white/50 hover:text-white transition-colors uppercase font-mono bg-transparent border-none cursor-pointer"
                      >
                    ...
                    <div className="text-sm sm:text-base text-white/85 leading-relaxed space-y-4 font-normal whitespace-pre-line">
                      {answer}
                    </div>
                    ...
                    {sources.length > 0 && (
                      <div className="mt-6 pt-5 border-t border-white/10 bg-black/40 p-4">
                        <p className="text-[10px] font-mono uppercase tracking-widest text-white/50 mb-2.5 flex items-center gap-1.5">
                    ...
                    {relatedArticles.length > 0 && (
                      <div className="mt-4 pt-4 border-t border-white/10 flex flex-wrap items-center gap-4 text-xs font-normal">
                        <span className="text-white/50 uppercase font-mono text-[10px] tracking-wider">
                          Recommended Reading:
                        </span>
                    ...
                    <p className="mt-5 text-[10px] text-white/35 font-mono leading-normal border-t border-white/5 pt-3">
