...
export default function CinematicComponentRunway() {
  const runwayItems = [
    {
      num: '01',
  ...
  return (
    <section className="py-32 px-6 sm:px-12 lg:px-24 bg-white text-alkota-black border-t border-[#E8E8E4]">
      <div className="max-w-7xl mx-auto space-y-20">
        
        {/* Section Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 pb-10 border-b border-[#E8E8E4]">
        ...
        {/* Unboxed 4-Column Discovery Runway */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-10">
          {runwayItems.map((item) => (
            <Link
              key={item.num}
              href={item.href}
              className="group flex flex-col justify-between space-y-6 no-underline text-inherit cursor-pointer"
            >
              {/* Product Visual Area on Soft Neutral Canvas */}
              <div className="relative aspect-square bg-[#FAF9F5] flex items-center justify-center p-8 overflow-hidden transition-all duration-500 group-hover:bg-[#F4F3EE]">
                <img
                  src={item.image}
                  alt={item.title}
